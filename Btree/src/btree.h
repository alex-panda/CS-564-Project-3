 ...
 * as page 2 but since a split can occur at the root the root page may get
 * moved up and get a new page no.
 */
...
/*
Each node is a page, so once we read the page in we just cast the pointer to
the page to this struct and use it to access the parts These structures
basically are the format in which the information is stored in the pages for
the index file depending on what kind of node they are. The level memeber of
each non leaf structure seen below is set to 1 if the nodes at this level are
just above the leaf nodes. Otherwise set to 0.
*/
...
class BTreeIndex {
 private:
  ...
  void insert(Page *currPage, PageId currPageId, bool isLeaf,
              RIDKeyPair<int> newEntry, PageKeyPair<int> *&newInternal);
  ...
  void splitLeaf(LeafNodeInt *leaf, PageId leafPageId,
                 PageKeyPair<int> *&newInternal, RIDKeyPair<int> newEntry);

  void insertLeaf(LeafNodeInt *leaf, RIDKeyPair<int> newEntry);

  void splitInternal(NonLeafNodeInt *oldNode, PageId oldPageId,
 ...
 public:
   ...
   * @param rid     Record ID of a record whose entry is getting inserted into
   *the index.
   **/
   ...
   **/
  void startScan(void *lowVal, const Operator lowOp, void *highVal,
